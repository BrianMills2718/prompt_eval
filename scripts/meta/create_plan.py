...
def render_plan(plan_number: int, title: str, blocked_by: str, blocks: str) -> str:
...
def create_plan(args: argparse.Namespace) -> dict[str, str | int | bool]:
    ...
    if args.dry_run:
        next_plan = preview_next_plan_number(repo_root=ROOT, fetch=not args.no_fetch)
        return {
            "dry_run": True,
            "plan": next_plan,
    ...
    if plan_path.exists():
        release_plan_reservation(repo_root=ROOT, plan=plan_number)
        raise ValueError(f"Plan file already exists: {plan_path}")

    original_index = INDEX_PATH.read_text(encoding="utf-8")
    try:
        plan_path.write_text(
            render_plan(plan_number, title, args.blocked_by, args.blocks),
            encoding="utf-8",
        )
